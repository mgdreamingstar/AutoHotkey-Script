...
hSysMenu := DllCall("GetSystemMenu", "UInt", hSpyWnd, "Int", False)
DllCall("InsertMenu", "UInt", hSysMenu, "UInt", 5, "UInt", 0x400, "UInt", 0xC0DE, "Str", "About...")
DllCall("InsertMenu", "UInt", hSysMenu, "UInt", 5, "UInt", 0xC00, "UInt", 0, "Str", "") ; Separator
...
Scintilla_GetFont(hWnd) {
    ...
    SendMessage 2486, 32, Address,, ahk_id %hWnd% ; SCI_STYLEGETSIZE, STYLE_DEFAULT
    If (ErrorLevel != "FAIL") {
...
LoadCursors() {
    ...
    For Key, Value in Constants {
        hCursor := DllCall("LoadCursor", "UInt", 0, "UInt", Value)
        Cursors[hCursor] := Key
    }
}
...
GetTabOrderIndex(hWnd) {
    ...
    Loop Parse, ControlList, `n
    {
        ...
        If (A_LoopField == hWnd && (Style & 0x10000)) {
            Return Index
        }
...
PostMsg:
    ...
    {
        IniRead Message, %A_ScriptDir%\Constants\Messages.ini, % GetClassName(g_hWnd), %CbxMessages%
    }

    DataTypes := {"Number": "UInt", "String": "WStr"}
...
GoogleSearch:
    ...
    Try {
        Run https://www.google.com/#q=%Message%
    }
Return
...
LoadProperties:
    ...
    DllCall("EnumPropsEx", "UInt", g_hWnd, "Ptr", Callback, "UInt", lParam := 0)
Return
...
CaptureWindow(hwndOwner, hwnd) {
    VarSetCapacity(RECT, 16, 0)
    DllCall("GetWindowRect", "Ptr", hwnd, "UInt", &RECT)
...
SetButtonIcon(hButton, File, Index := 1) {
    himl := DllCall("ImageList_Create", "UInt", 16, "UInt", 16, "UInt", 0x20, "UInt", 1, "UInt", 1) ; ILC_COLOR32
...
FindWindow:
    ...
    Loop %WinList% {
        ...
        If (MatchCriteria(WTitle, WClass, WProcess)) {
        ...
        Loop Parse, ControlList, `n
        {
            ...
            If (MatchCriteria(CText, CClass, CProcess)) {
...
MatchCriteria(Text, Class, Process) {
    ...
    If (CbxFindByProcess != "" && !InStr(Process, CbxFindByProcess)) {
        Match := False
    }
...
ShowMenuViewer:
    If (MenuViewerExist) {
        ...
        Gui MenuViewer: New, LabelMenuViewer hWndhMenuViewer
        Gui Font, s9, Segoe UI
        Gui Color, White

        Gui Add, ListView, hWndhMenuList x8 y10 w583 h391 +LV0x14000, Menu Item String|Keyboard|Menu ID
        LV_ModifyCol(1, 396)
        LV_ModifyCol(2, 100)
        LV_ModifyCol(3, "65 Integer")
        SetExplorerTheme(hMenuList)

        Gui Add, Text, x-1 y400 w602 h48 +Border -Background
        ...
        DllCall("SetParent", "UInt", hSearchIcon, "UInt", hEdtMenuSearch)
        WinSet Style, -0x40000000, ahk_id %hSearchIcon% ; -WS_CHILD
        ControlFocus,, ahk_id %hEdtMenuSearch%

        NoAmpersands := True
        Gui Add, CheckBox, vNoAmpersands x220 y408 w167 h23 -Background Checked%NoAmpersands%
        , Remove Ampersands (&&)
        Gui Add, Button, gCopyMenuList x466 y407 w125 h25, Copy to Clipboard
...
LoadProcessProperties:
    ...
    If (Enum[Process]) {
        ...
        If (!hIcon || ExePath == "") {
            hIcon := DllCall("LoadIcon", "UInt", 0, "UInt", 32512) ; IDI_APPLICATION
        }

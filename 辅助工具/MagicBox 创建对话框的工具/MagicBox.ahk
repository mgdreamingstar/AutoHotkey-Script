...
#Include Lib\ResourceId.ahk
#Include Lib\AutoXYWH.ahk
#Include Lib\ExecScript.ahk
#Include Lib\ControlColor.ahk
#Include Lib\CommonDialogs.ahk

#Include Functions\MagicBox.ahk
#Include Functions\MessageBoxCheck.ahk
#Include Functions\MessageBoxIndirect.ahk
#Include Functions\SoftModalMessageBox.ahk
#Include Functions\MsiMessageBox.ahk
#Include Functions\WTSSendMessage.ahk
#Include Functions\TaskDialog.ahk
...
If (FileExist("MagicBox.ini")) {
    IniRead MsgBoxType, MagicBox.ini, MagicBox, Type, MsgBox
    IniRead Title, MagicBox.ini, MagicBox, Title
    IniRead Text, MagicBox.ini, MagicBox, Text
    IniRead Content, MagicBox.ini, MagicBox, Content
}
...
Gui Add, DropDownList, vMsgBoxType gSetMsgBoxType x466 y24 w133, % (NT6) ? "MagicBox|MsgBox||SHMessageBoxCheck|MessageBoxIndirect|SoftModalMessageBox|MsiMessageBox|WTSSendMessage|TaskDialog|TaskDialogIndirect" : "MagicBox|MsgBox||SHMessageBoxCheck|MessageBoxIndirect|SoftModalMessageBox|MsiMessageBox|WTSSendMessage"

Gui Add, Tab2, vHiddenTab x0 y0 w0 h0 -TabStop, 1|TaskDialogIndirect|SoftModalMessageBox|MagicBox
...
Gui Add, Button, vExitBtn gMagicBoxClose x535 y430 w75 h23, E&xit
...
FileRead _MagicBox,            Functions\MagicBox.ahk
...
DllCall("AppendMenu", "UInt", hSysMenu, "UInt", 0x800, "UInt", 0, "Str", "") ; Separator
DllCall("AppendMenu", "UInt", hSysMenu, "UInt", 0, "UInt", 0xDEAD, "Str", "More Options...`tCtrl+O") 
DllCall("AppendMenu", "UInt", hSysMenu, "UInt", 0, "UInt", 0xC0DE, "Str", "Buttons in English`tCtrl+E")
...
Test:
    ...
    ; WTSSendMessage
        ...
        If (!WTS_Wait) {
            MsgBox 0, MagicBox, Response: 32001 (IDASYNC)
        }
...
SetMsgBoxType:
    ...
    If (MsgBoxType == "MagicBox" || MsgBoxType == "SoftModalMessageBox" || MsgBoxType == "TaskDialogIndirect") {
...
ViewCode:
    If (!DllCall("IsWindow", "UInt", hCodeView)) {
        ...
        Gui Add, CheckBox, vIfBlocks gGenerateCode x8 y307 w333 h23, &Generate conditional statement blocks for the return value
        Gui Add, Button, gSaveCode x354 y307 w75 h23, &Save 
        Gui Add, Button, gCopyCode x435 y307 w75 h23, &Copy
        Gui Add, Button, gCancel x516 y307 w75 h23, C&ancel
        Gui Show, w599 h343, MagicBox Factory - Code
        Gui +MinSize599
    } Else {
        Gui CodeView: Show
    }
...
GenerateCode:
    ...
    If (MsgBoxType == "MsgBox") {
            ...
            If (MB_Custom) {
                ...
                If (CustomIcon) {
                    Code .= "        hIcon := LoadPicture(""" . IconRes
                         ...
                         .  "        SendMessage 0x172, 1, %hIcon% , Static1`n"
                }
        ...
        cButtons := SM_CustomButtons.MaxIndex()
        Code .= "Buttons := ["
        SM_Callback := False
        Loop % cButtons {
        ...
        If (SM_Callback) {
            Code .= "SoftModalCallback() {`n    MsgBox`n}`n`n"
        }
...
OnWM_KEYDOWN(wParam, lParam, msg, hWnd) {
    ...
    If (wParam == 9) {
        ...
        If (!ShiftP && A_GuiControl == "Title") {
            ...
            Return False
        } Else If (!ShiftP && A_GuiControl == "ExitBtn") {
...
TDI_Test:
    ...
    ; Radio buttons
    cRadioButtons := RadioButtons.MaxIndex()
    If (cRadioButtons) {
        ...
        Loop % cRadioButtons {
            ...
            NumPut(iRadioID,   pRadioButtons, (4 + A_PtrSize) * (A_Index - 1))
            NumPut(iRadioText, pRadioButtons, (4 + A_PtrSize) * A_Index - A_PtrSize)
        }

        NumPut(cRadioButtons, TDC, (x64) ? 76 : 48) ; cRadioButtons
        NumPut(&pRadioButtons, TDC, (x64) ? 80 : 52) ; pRadioButtons
    }
    ...
    If (TDI_AlwaysOnTop || TDI_TimeoutEnabled || TDI_Marquee) {
        ...
        NumPut(TDI_Callback, TDC, (x64) ? 140 : 84) ; pfCallback
    ...
    If (TDI_RelativePosition) {
        ...
        hParent := DllCall("GetDesktopWindow")
    } Else {
        hParent := 0
    }

    ; TASKDIALOGCONFIG structure
    NumPut(TDCSize, TDC, 0)                                  ; cbSize
    NumPut(hParent, TDC, 4)                                  ; hwndParent
    ;NumPut(hModule, TDC, (x64) ? 12 : 8)                    ; hInstance
    NumPut(TDI_Flags, TDC, (x64) ? 20 : 12)                  ; dwFlags
    NumPut(TDI_Buttons, TDC, (x64) ? 24 : 16)                ; dwCommonButtons
    NumPut(&Title, TDC, (x64) ? 28 : 20)                     ; pszWindowTitle
    NumPut(TDI_MainIcon, TDC, (x64) ? 36 : 24)               ; pszMainIcon
    NumPut(&TDI_Instruction, TDC, (x64) ? 44 : 28)           ; pszMainInstruction
    NumPut(&TDI_Content, TDC, (x64) ? 52 : 32)               ; pszContent
    NumPut(TDI_DefaultButton, TDC, (x64) ? 72 : 44)          ; nDefaultButton
    NumPut(TDI_DefaultRadio, TDC, (x64) ? 88 : 56)           ; nDefaultRadioButton
    NumPut(&TDI_VerificationText, TDC, (x64) ? 92 : 60)      ; pszVerificationText
    NumPut(&TDI_ExpandedText, TDC, (x64) ? 100 : 64)         ; pszExpandedInformation
    NumPut(&TDI_ExpandedControlText, TDC, (x64) ? 108 : 68)  ; pszExpandedControlText
    NumPut(&TDI_CollapsedControlText, TDC, (x64) ? 116 : 72) ; pszCollapsedControlText
    NumPut(TDI_FooterIcon, TDC, (x64) ? 124 : 76)            ; pszFooterIcon
    NumPut(&TDI_FooterText, TDC, (x64) ? 132 : 80)           ; pszFooter
    ...
    If (TDI_Callback) {
        DllCall("Kernel32.dll\GlobalFree", "UInt", TDI_Callback)
    }
Return
...
TDI_GenerateCode:
    ...
    If (cButtons) {
        ...
        Code .= "cButtons := CustomButtons.MaxIndex()`n"
             ...
             .  "    NumPut(iButtonID,   pButtons, (4 + A_PtrSize) * (A_Index - 1))`n"
             .  "    NumPut(iButtonText, pButtons, (4 + A_PtrSize) * A_Index - A_PtrSize)`n"
             .  "}`n"
    }
    ...
    If (cRadioButtons) {
        ...
        Code .= "cRadioButtons := RadioButtons.MaxIndex()`n"
             ...
             .  "    NumPut(iButtonID,   pRadioButtons, (4 + A_PtrSize) * (A_Index - 1))`n"
             .  "    NumPut(iButtonText, pRadioButtons, (4 + A_PtrSize) * A_Index - A_PtrSize)`n"
             .  "}`n"
    }
    ...
    If (TDI_RelativePosition) {
        ...
        Code .= "Parent := DllCall(""GetDesktopWindow"")`n"
    }
    ...
    (LTrim
        ...
        x64 := A_PtrSize == 8
        NumPut(VarSetCapacity(TDC, (x64) ? 160 : 96, 0), TDC, 0) `; cbSize`n"
    )

    If (TDI_RelativePosition || TDI_Cancel) {
        Code .= "NumPut(Parent, TDC, 4) `; hwndParent`n"
    }

    If (TDI_Flags) {
        Code .= "NumPut(Flags, TDC, (x64) ? 20 : 12) `; dwFlags`n"
    }

    If (TDI_Buttons) {
        Code .= "NumPut(Buttons, TDC, (x64) ? 24 : 16) `; dwCommonButtons`n"
    }

    If (Title != "") {
        Code .= "NumPut(&Title, TDC, (x64) ? 28 : 20) `; pszWindowTitle`n"
    }

    If (TDI_MainIcon != 0) {
        Code .= "NumPut(MainIcon, TDC, (x64) ? 36 : 24) `; pszMainIcon`n"
    }

    If (TDI_Instruction != "") {
        Code .= "NumPut(&Instruction, TDC, (x64) ? 44 : 28) `; pszMainInstruction`n"
    }

    If (TDI_Content != "") {
        Code .= "NumPut(&Content, TDC, (x64) ? 52 : 32) `; pszContent`n"
    }

    If (cButtons) {
        Code .= "NumPut(cButtons, TDC, (x64) ? 60 : 36) `; cButtons`n"
        Code .= "NumPut(&pButtons, TDC, (x64) ? 64 : 40) `; pButtons`n"
    }

    If (TDI_DefaultButton != 0) {
        Code .= "NumPut(DefaultButton, TDC, (x64) ? 72 : 44) `; nDefaultButton`n"
    }

    If (cRadioButtons) {
        Code .= "NumPut(cRadioButtons, TDC, (x64) ? 76 : 48) `; cRadioButtons`n"
        Code .= "NumPut(&pRadioButtons, TDC, (x64) ? 80 : 52) `; pRadioButtons`n"

        If (TDI_DefaultRadio != 0) {
            Code .= "NumPut(DefaultRadio, TDC, (x64) ? 88 : 56) `; nDefaultRadioButton`n"
        }
    }

    If (TDI_VerificationText != "") {
        Code .= "NumPut(&CheckText, TDC, (x64) ? 92 : 60) `; pszVerificationText`n"
    }

    If (TDI_ExpandedText != "") {
        Code .= "NumPut(&ExpandedText, TDC, (x64) ? 100 : 64) `; pszExpandedInformation`n"
    }

    If (TDI_ExpandedControlText != "") {
        Code .= "NumPut(&ExpandedControlText, TDC, (x64) ? 108 : 68) `; pszExpandedControlText`n"
    }

    If (TDI_CollapsedControlText != "") {
        Code .= "NumPut(&CollapsedControlText, TDC, (x64) ? 116 : 72) `; pszCollapsedControlText`n"
    }

    If (TDI_FooterIcon != 0 && TDI_FooterText != "") {
        Code .= "NumPut(FooterIcon, TDC, (x64) ? 124 : 76) `; pszFooterIcon`n"
    }

    If (TDI_FooterText != "") {
        Code .= "NumPut(&FooterText, TDC, (x64) ? 132 : 80) `; pszFooter`n"
    }

    If (Callback) {
        Code .= "NumPut(TDCallback, TDC, (x64) ? 140 : 84) `; pfCallback`n"
        Code .= "NumPut(&CBData, TDC, (x64) ? 148 : 88) `; lpCallbackData`n"
    }
    ...
    Code .= "`nDllCall(""Comctl32.dll\TaskDialogIndirect"", ""UInt"", &TDC`n"
         .  "    , ""Int*"", Button := 0`n"
         .  "    , ""Int*"", Radio := 0`n"
         .  "    , ""Int*"", Checked := 0)`n"

    If (Callback) {
        Code .= "`nDllCall(""Kernel32.dll\GlobalFree"", ""UInt"", TDCallback)`n"
    }
...
LoadButtonText(ID) {
    ...
    If (DllCall("LoadString", "UInt", hModule, "UInt", ID, "Str", Buffer, "UInt", 42)) {
...
OnWM_ENTERMENULOOP(wParam, lParam, msg, hWnd) {
    ...
    DllCall("EnableMenuItem", "Ptr", hSysMenu, "UInt", 0xDEAD, "UInt", Disable)
    Disable := MsgBoxType ~= "^(Magic|Soft|TaskDialogIndirect)"
    DllCall("EnableMenuItem", "Ptr", hSysMenu, "UInt", 0xC0DE, "UInt", Disable)
}
...
SwitchButtonText(Eng) {
    ...
    DllCall("CheckMenuItem", x64 ? "Ptr" : "UInt", hSysMenu, "UInt", 0xC0DE, "UInt", Eng * 8)
...
SM_UpdateDefaultButtonList:
    Buttons := ""
    Loop % SM_CustomButtons.MaxIndex() {
...
MBX_GenerateCode:
    ...
    Code .= "Result := MagicBox(Text" . Call . ")`n`n"
...
MBX_GenerateIfBlocks:
    ...
    If (!InStr(StrReplace(MBX_Buttons, "&"), "Cancel")) {
        If (Ifs != "") {
            Ifs .= " Else "
...
MBX_SelectFont:
    If (ChooseFont(MBX_FontName, MBX_FontOptions, True, hWindow)) {
        MBX_CustomFont := True
    } Else {
        MBX_CustomFont := False
        MBX_FontOptions := ""
...
MBX_Callback:
    MsgBox 0x40040, %AppName%, MagicBox callback invoked., 3
Return
...
ShowHelp() {
    ...
    Loop 8 {
        DllCall(SetClassLong, "Ptr", hLink%A_Index%, "int", -12, "UInt", hCursor)
    }
